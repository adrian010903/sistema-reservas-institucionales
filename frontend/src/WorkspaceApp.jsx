...
function Header({ user, page, navigate, logout, unreadCount }) {
  ...
  return <header className="app-nav"><button className="app-brand official-brand" aria-label="Ir al inicio" onClick={() => navigate(user ? 'dashboard' : 'home')}><img src="/assets/logo-guias-scouts-cr.png" alt="Guías y Scouts de Costa Rica" /></button><nav className={user ? 'authenticated-nav' : 'public-nav'}>{items.map(([key, label]) => <button className={`nav-link nav-${key} ${page === key ? 'active' : ''}`} key={key} onClick={() => navigate(key)}><span className="nav-icon" aria-hidden="true">{key === 'home' && <svg viewBox="0 0 24 24" focusable="false"><path d="M3 10.8 12 3l9 7.8v9.2a1 1 0 0 1-1 1h-5.2v-6h-5.6v6H4a1 1 0 0 1-1-1z" /></svg>}{key === 'spaces' && <svg viewBox="0 0 24 24" focusable="false"><path d="M6 3.5h12v17l-6-3.4-6 3.4z" /></svg>}</span><span>{label}</span>{key === 'spaces' && !user && <span className="nav-chevron" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><path d="m6 9 6 6 6-6" /></svg></span>}</button>)}</nav>{user ? <button className="nav-session" onClick={logout}>Cerrar sesión</button> : <button className="nav-session public-login" onClick={() => navigate('login', page)}>Iniciar sesión</button>}</header>
}
...
function WorkspaceApp() {
  ...
  async function reviewReservation(event) {
    ...
    const params = new URLSearchParams({ fecha: form.fecha, horaInicio: form.horaInicio, horaFin: form.horaFin, personas: form.cantidadPersonas, lugarId: selectedSpace.lugarId })
    const response = await fetch(`${API}/reservas/disponibilidad?${params}`)
    const available = await response.json().catch(() => [])
    setCheckingReservation(false)
    if (!response.ok) return setMessage(available.detail || available.message || 'No se pudo comprobar la disponibilidad')
    if (!available.some(space => space.id === selectedSpace.id)) { setReservationAvailability('unavailable'); return setMessage('Ese espacio ya está ocupado en el horario seleccionado. Elige otra hora.') }
  ...
  function renderReservePage() {
    ...
    return <main className="page-container reserve-page"><p className="eyebrow">Nueva reserva</p><h1>Configura tu visita</h1><p>Primero elige el lugar y después el espacio que necesitas. Horario institucional: 08:00 a 17:00.</p>
      <form className="reservation-form polished visual-reservation-form" onSubmit={reviewReservation}>
        ...
        <div className="selector-heading schedule-heading"><span>3</span><div><strong>Define fecha y horario</strong><small>Comprobaremos automáticamente que el espacio continúe disponible</small></div></div><label>Fecha<input type="date" min={new Date().toISOString().slice(0,10)} required value={form.fecha} onChange={e => { setForm({...form, fecha:e.target.value}); setReservationAvailability(null) }}/></label><label>Hora inicio<input type="time" min="08:00" max="17:00" required value={form.horaInicio} onChange={e => { setForm({...form, horaInicio:e.target.value}); setReservationAvailability(null) }}/></label><label>Hora fin<input type="time" min="08:00" max="17:00" required value={form.horaFin} onChange={e => { setForm({...form, horaFin:e.target.value}); setReservationAvailability(null) }}/></label><label>Personas<input type="number" min="1" max={selectedSpace?.capacidad || undefined} required value={form.cantidadPersonas} onChange={e => { setForm({...form, cantidadPersonas:e.target.value}); setReservationAvailability(null) }}/></label><div className="live-total reservation-live-total"><div><span>Duración</span><strong>{hours || 0} hora(s)</strong></div><div><span>Tarifa por hora</span><strong>₡{hourlyRate.toLocaleString('es-CR')}</strong></div><div><span>Total provisional</span><strong>₡{(hours * hourlyRate).toLocaleString('es-CR')}</strong></div></div>{reservationAvailability === 'available' && <div className="availability-result available">✓ Horario disponible</div>}{reservationAvailability === 'unavailable' && <div className="availability-result unavailable">× Horario ocupado</div>}<button className="primary-button reserve-review-button" disabled={!selectedSpace || checkingReservation}>{checkingReservation ? 'Comprobando disponibilidad…' : 'Comprobar y revisar reserva'}</button>
  ...
  return <div className="workspace-app"><Header user={user} page={page} navigate={navigate} logout={logout} unreadCount={notifications.filter(notification => !notification.leida).length} />
    ...
    {showSummary && <div className="custom-modal-overlay"><section className="custom-modal reservation-review-modal"><button className="custom-modal-close" onClick={() => setShowSummary(false)}>×</button><div className="review-cover" style={selectedSpace?.imagenUrl ? {backgroundImage:`linear-gradient(0deg,rgba(33,11,77,.72),transparent),url(${BACKEND}${selectedSpace.imagenUrl})`} : undefined}><span>Horario disponible</span><h2>{selectedSpace?.nombre}</h2></div><p className="eyebrow">Resumen de reserva</p><div className="review-location"><strong>{places.find(place => String(place.id) === String(selectedSpace?.lugarId))?.nombre}</strong><small>{selectedSpace?.tipo} · {selectedSpace?.categoria}</small></div><div className="modal-summary-grid"><div><small>Fecha</small><strong>{new Date(`${form.fecha}T12:00`).toLocaleDateString('es-CR',{day:'2-digit',month:'long',year:'numeric'})}</strong></div><div><small>Horario</small><strong>{form.horaInicio} - {form.horaFin}</strong></div><div><small>Duración</small><strong>{hours} hora(s)</strong></div><div><small>Personas</small><strong>{form.cantidadPersonas}</strong></div></div><div className="modal-total"><span>₡{hourlyRate.toLocaleString('es-CR')} × {hours} hora(s)</span><strong>₡{(hours * hourlyRate).toLocaleString('es-CR')}</strong><small>Total provisional en colones costarricenses</small></div><div className="custom-modal-actions"><button className="secondary-button" onClick={() => setShowSummary(false)}>Modificar</button><button className="primary-button" onClick={confirmReservation}>Confirmar y continuar al pago</button></div></section></div>}
